...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
	...
	<input type="hidden" id="{$fields.state.name}_multiselect"
	name="{$fields.state.name}_multiselect" value="true">
	{multienum_to_array string=$fields.state.value default=$fields.state.default assign="values"}
	<select id="{$fields.state.name}"
	name="{$fields.state.name}[]"
	multiple="true" size='6' style="width:150" title='' tabindex="1"  
     	>
	{html_options options=$fields.state.options selected=$values}
	</select>
...
{else}
	...
	{assign var="ac_key" value=$smarty.capture.ac_key}
		...
		name="{$fields.state.name}_multiselect" value="true">
		{multienum_to_array string=$fields.state.value default=$fields.state.default assign="values"}
		<select style='display:none' id="{$fields.state.name}"
		name="{$fields.state.name}[]"
		multiple="true" size='6' style="width:150" title='' tabindex="1"  
		        >
		{html_options options=$fields.state.options selected=$values}
	...
	<span class="id-ff multiple">
	    <button type="button">
	    	<img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.state.name}-image">
	    	</button>
	    	<button type="button"
	        id="btn-clear-{$fields.state.name}-input"
	        title="Clear"
	        onclick="SUGAR.clearRelateField(this.form, '{$fields.state.name}-input', '{$fields.state.name};');SUGAR.AutoComplete.{$ac_key}.inputNode.updateHidden()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
	</span>
	...
	YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters","node-event-simulate", function (Y) {
		...
		if({/literal}SUGAR.AutoComplete.{$ac_key}.minQLen{literal} == 0){
		    ...
		    function updateTextOnInterval(){
		    	if (document.activeElement != document.getElementById("{/literal}{$fields.state.name}-input{literal}"))
