...
class AORScheduledReportJob implements RunnableSchedulerJob
{
    ...
    public function run($data)
    {
        ...
        //  $GLOBALS['log']->fatal(print_r($report,true)."report bean");
        //$GLOBALS['log']->fatal($bean->aor_report_id."report bean id");
        $report_id = $bean->aor_report_id;
        $aor_scheduled_reportid = $bean->id;
        $unique_timestamp = mktime();
$js=<<<EOD
        var page = require('webpage').create();
page.open('{$sugar_config['site_url']}/index.php?entryPoint=SendReportsinEmail&record={$report_id}&aor_scheduled_reportid={$aor_scheduled_reportid}&unique_timestamp={$unique_timestamp}', function (status) {
    setTimeout(function () {
        phantom.exit();
    }, 20000);
});
EOD;
        //  $GLOBALS['log']->fatal(print_r(dirname(__FILE__) ,true)."upload dir");
       // $unique_name = 'upload/scheduled_reports/phantomjs_'.$unique_timestamp .'.js';
        $unique_name = 'upload/scheduled_reports/phantomjs_'.$unique_timestamp .'.js';
        $phantomscript = fopen($unique_name, "w") or die("Unable to open file!");
        fwrite($phantomscript, $js);
        fclose($phantomscript);
        $GLOBALS['log']->fatal('phantomjs --ssl-protocol=any '.$unique_name);
        exec('phantomjs --ssl-protocol=any '.$unique_name);
        exit;
        /*Modified By Swapnil End*/
