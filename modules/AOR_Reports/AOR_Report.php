...
//                }
    ...
    function build_group_report($offset = -1, $links = true, $extra = array()) {
        ...
        if ($field_id != '') {
            ...
            $row_list = array();
            while ($row = $this->db->fetchByAssoc($result)) {
				if(!in_array($row[$field_label] , $row_list)) {
					if ($html != '') {
						$html .= '<br />';
					}
    ...
    function build_report_html($offset = -1, $links = true, $group_value = '', $tableIdentifier = '', $extra = array()) {
        ...
        if ($offset >= 0) {
            ...
            <a  id='listViewPrevButton_top' name='listViewPrevButton' title='Previous' onclick='changeReportPage(\"" . $this->id . "\"," . $previous_offset . ",\"" . $group_value . "\",\"" . $tableIdentifier . "\")'>
    ...
    private function getModuleFieldByGroupValue($beanList, $group_value) {
        ...
        while ($row = $this->db->fetchByAssoc($result)) {
			...
			//Group name was showing 36 bit assigned_user_id. to get user name, following condition was added. By Shakeer 31st Jan 2018
            if (($field->field_function != 'COUNT' || $field->format != '') && (strlen($group_value) != 36)) {
            ...
            while ($row = $this->db->fetchByAssoc($result)) {
                ...
                if (isset($aor_sql_operator_list[$condition->operator])) {
                    ...
                    if (!empty($this->user_parameters[$condition->id]) && $condition->parameter) {
                        ...
                        //Added to resolve pagination issue for Datetime parameter by Shakeer on 30 Jan 2018
                        $operators_list = ['Equal_To' , 'Greater_Than_or_Equal_To' , 'Less_Than' , 'Greater_Than' , 'Less_Than_or_Equal_To'];
                        
                        if(in_array($condParam['operator'] , $operators_list) && preg_match("/([0-9]{2}|[0-9]{4})[- \/.]([0-9]{2})[- \/.]([0-9]{2}|[0-9]{4})/", $condParam['value'])) {
							global $current_user;
							$dateFormat = $current_user->getPreference('datef');
							
                                                         $new_value = date("d-m-Y", strtotime($condParam['value']));
							 $value = DateTime::createFromFormat("d-m-Y" , $new_value)->format("Y-m-d");
                    ...
                    switch ($condition->value_type) {
                        ...
                        case "CurrentUserID":
                            ...
                            break;
                        case 'Value':
                            //print_r($condition->value."---->".$condition->operator.">>>>>>".$condition->condition_order);
                            //exit;
                            /*Modidied By Shakeer and Manasa*/
                            if ($condition->operator == 'Equal_To') {
                                //echo "hello";
                                ...
                                if ($data['type'] == 'datetime' || $data['type'] == 'datetimecombo') {
                                    ...
                                    } else if (preg_match("/(0[1-9]|1[012])[- \/.](0[1-9]|[12][0-9]|3[01])[- \/.](19|20)\d\d/", $condition->value)) {
                                        $Date = explode("/", $condition->value);
                                        $new_date = $Date[2] . "-" . $Date[0] . "-" . $Date[1];
                                        $condition->value = date("Y-m-d 18:29:59", strtotime($new_date));
                                    } else if (preg_match("/(0[1-9]|[12][0-9]|3[01])[\/.](0[1-9]|1[012])[\/.](19|20)\d\d/", $condition->value)) {
                                        $Date = explode("/", $condition->value);
                                ...
                                if ($data['type'] == 'datetime' || $data['type'] == 'datetimecombo') {
                                    if (preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", $condition->value)) {
                                        ...
                                        $new_date = $Date[2] . "-" . $Date[1] . "-" . $Date[0];
                                        $condition->value = date("Y-m-d 18:29:59", strtotime($new_date));
                                    } else if (preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])/", $condition->value)) { // YYYY-mm_-dd HH:ii:ss format. add one day to the values. Shakeer on 20 Mar 2018
                                        $condition->value = date("Y-m-d 18:29:59", strtotime("+1 day" , strtotime($condition->value)));
                                    }
                                }
                                $value = "'" . $this->db->quote($condition->value) . "'";
                            } else {
