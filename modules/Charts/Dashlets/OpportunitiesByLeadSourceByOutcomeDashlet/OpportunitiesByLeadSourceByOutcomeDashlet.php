...
/*smith*/
$jsonKey = json_decode($jsonKey, true);
array_unshift($jsonKey,"Genre");
$jsonKey = json_encode($jsonKey);
$jsonLabels = json_decode($jsonLabels, true);
//print_r($jsonKey);
$jsonData = json_decode($jsonData, true);
$new_array = '';
foreach ($jsonData as $index =>$arr)
{
array_unshift($arr,$jsonLabels[$index]);
...
function drawChart() {
   ...
   var chart = new google.visualization.BarChart(document.getElementById('$canvasId'));
...
<div id="$canvasId" style="width: '100%'; height: '$chartHeight'; margin: 0 auto"></div> 
EOD;
