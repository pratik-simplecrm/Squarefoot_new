...
//echo ' name '. print_r($name);
	...
	for($i=0; $i< count($name); $i++){
		...
		$query8 ="SELECT id FROM  securitygroups WHERE name ='$name[$i]' and deleted=0"; 
		...
		if($admin == 0){
			...
			for ($j=0;$j<count($sub_id); $j++){
				$getarch ="SELECT ac.id,ac.assigned_user_id,ac.last_name,ac.first_name  FROM  arch_architects_contacts ac LEFT JOIN securitygroups_records sg ON ac.id = sg.record_id WHERE sg.securitygroup_id = '$team_id' and ac.assigned_user_id='$sub_id[$j]' and ac.deleted='0'"; 
		 	...
		 	$getarch ="SELECT a.id,a.assigned_user_id,a.last_name,a.first_name  FROM  arch_architects_contacts a LEFT JOIN securitygroups_records sg ON a.id = sg.record_id WHERE sg.securitygroup_id = '$team_id' and a.deleted='0'";
			$resgetteam=$db->query($getarch);
			while($rowgetpayment=$db->fetchByAssoc($resgetteam))
			{
				$archid=$rowgetpayment['id'];
