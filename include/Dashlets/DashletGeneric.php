...
class DashletGeneric extends Dashlet {
    ...
    function processDisplayOptions() {
        ...
        foreach ($this->columns as $fkey => $fval) {
            $mtranslated = translate($fval['label'], $this->seedBean->module_dir);
            if (is_array($mtranslated))
                $mtranslated = $fval['label'];
            $all_columns[$fkey] = $mtranslated;
        }
        //
        global $db, $sugar_config;
        $main_query = $db->query("SELECT * from dashlet_360_hide_show_fields where dashlet_id='" . $this->id . "'");
        $main_data = $db->fetchByAssoc($main_query);


        $this->configureSS->assign('hide_show_fields', unserialize(base64_decode($main_data['display_hide_columns'])));
        $full_array = unserialize(base64_decode($main_data['display_hide_columns']));

        $merge_hd_ds = array_merge($full_array['dis'], $full_array['hid']);

        $hd_sh_col = array_diff(array_flip($all_columns), $merge_hd_ds);

        $this->configureSS->assign('hd_sh_col', array_flip($hd_sh_col));

        $this->configureSS->assign('all_columns', $all_columns);
    ...
    function buildWhere() {
        ...
        /* -----------360 dynamic dashlets condition By Roshan Sarode 27/05/2017 start -------- */
        global $db, $sugar_config, $current_user;

        if ($_SESSION[$current_user->user_name . "_PREFERENCES"]['Home']['pages'][1]['pageTitle'] == "Customer 360") {
            foreach ($_SESSION[$current_user->user_name . "_PREFERENCES"]['Home']['pages'][1]['columns'] as $col_id) {
...
//print_r($current_user);
        ...
        if (!in_array($this->id, $all_get_id)) {
            if ($this->myItemsOnly)
                array_push($returnArray, $this->seedBean->table_name . '.' . "assigned_user_id = '" . $current_user->id . "'");
        }

        /* -----------360 dynamic dashlets condition By Roshan Sarode 27/05/2017 end -------- */
        return $returnArray;
    }
    ...
    function process($lvsParams = array(), $id = null) {
        ...
        if (!empty($this->displayTpl)) {
            ...
            if (in_array($this->id, $all_get_id)) {
                ...
                $id_query = $db->query("SELECT * from retail_customer_dashlet where user_id='" . $current_user->id . "'");
                while ($get_id = $db->fetchByAssoc($id_query)) {

                    $bean = BeanFactory::getBean("Accounts");
                    $field_defs["Accounts"] = $bean->getFieldDefinitions();


                    foreach ($field_defs as $val) {
                        foreach ($val as $inke => $inval) {


                            if ($inval['bean_name'] == $this->seedBean->object_name || $this->seedBean->object_name == "Document" || $this->seedBean->object_name == "Case") {
                                $my_module = new Account();

                                // echo $this->seedBean->object_name."<br>";



                                $my_module->retrieve($get_id['retail_customer_id']);

//echo "<pre>";
//echo $inval['name']."==".$inval['bean_name']."<br>";
// 			
                                $contacts = $my_module->get_linked_beans($inval['name'], $inval['bean_name']);


                                foreach ($contacts as $contact) {
                                    $all_ids[] = "'" . $contact->id . "'";
                                }
                                $implode_ids = implode(", ", $all_ids);
                                $custom_where_new = " " . $this->seedBean->table_name . ".id IN (" . $implode_ids . ") ";
                            }
                        }
                    }
                }
                $whereArray[] = $custom_where_new;
            }
            /* -----------360 dynamic dashlets condition By Roshan Sarode 27/05/2017 start -------- */
