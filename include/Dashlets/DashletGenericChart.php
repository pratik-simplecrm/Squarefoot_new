...
//	$additionalTitle.='<a href="javascript:void(0)" class="flip_open"><i class="fa fa-arrows-h" aria-hidden="true"></i></a>&nbsp;&nbsp;';
        ...
        if ( is_array($this->_searchFields) ) {
            foreach($this->_searchFields as $name=>$params) {
                if(!empty($name)) {
                    ...
                    if ($name == 'obmdmtwrt_team_name') {
                        global $db;
                        $result = $db->query("select * from securitygroups where deleted='0'");
                        $teams .='<select name="obmdmtwrt_team_name[]" multiple size="3">';
                        $teams .= "<option value=''></option>";
                        while ($row = $db->fetchByAssoc($result)) {
                            $teams .= "<option value='" . $row['id'] . "'";
                            if (in_array($row['id'], $widgetDef['input_name0'])) {
                                $teams .= " selected='selected'";
                            }
                            $teams .= ">" . $row['name'] . "</option>";
                            }
                        $teams .='</select>';
                        $currentSearchFields[$name]['input'] = $teams;
                    }else if ($name == 'obmdmtuw_ids') {
                        global $db,$current_user;
                        $logged_in_user=$current_user->id;
                         $users .='<select name="obmdmtuw_ids[]"  size="3">';
                        $users .= "<option value=''></option>";
                        if($current_user->is_admin==1){
                        $result = $db->query("select * from users where deleted='0'");
                        }else{
                             $result = $db->query("select * from users where reports_to_id In ('" . $logged_in_user . "') and deleted=0");
         $users .= "<option value='$logged_in_user'";
                         if (in_array($logged_in_user, $widgetDef['input_name0'])) {
                        $users .=" selected='selected' ";     
                         }
                        $users .= ">$current_user->user_name</option>";
                        }
                       
                        
                      
                        while ($row = $db->fetchByAssoc($result)) {
                            $users .= "<option value='" . $row['id'] . "'";
                            if (in_array($row['id'], $widgetDef['input_name0'])) {
                                $users .= " selected='selected'";
                            }
                            $users .= ">" . $row['user_name'] . "</option>";
                            }
                        $users .='</select>';
                        $currentSearchFields[$name]['input'] = $users;
                    }  else {
