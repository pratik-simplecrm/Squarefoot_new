...
<div id="dashletSearch">
	<table>
		<tr>
			...
			<input type="button" class="button " style="background-color:#2767A8 !important;color:white !important;"  value="{sugar_translate label='LBL_SEARCH' module='Home'}" onClick="javascript:SUGAR.mySugar.searchDashlets(document.getElementById('search_string').value,document.getElementById('search_category').value);" />
			<input type="button" class="button " style="background-color:#2767A8 !important;color:white !important;" value="{sugar_translate label='LBL_CLEAR' module='Home'}" onClick="javascript:SUGAR.mySugar.clearSearch();" />			
			{if $moduleName == 'Home'}
...
{literal}
<style type="text/css">
	div#moduleDashletsList i {
    vertical-align: middle;
}
/*Module Icons changes made by Roshan Sarode*/
.adddashlettable tbody tr td{
    padding:4px;
}
</style>{/literal}
{if $moduleName == 'Home'}
<div id="moduleDashlets" style="height:400px;display:;">
	...
	<div id="moduleDashletsList" style="height:3913px;overflow:auto;display:;">
            {*Module Icons changes made by Roshan Sarode*}
	<table width="95%" class="adddashlettable">
		{counter assign=rowCounter start=0 print=false}
		{foreach from=$modules item=module}
		{if $rowCounter % 2 == 0}
		<tr>
		{/if}
			<td width="50%" align="left" valign="middle"><a id="{$module.id}_icon" href="javascript:void(0)" onclick="{$module.onclick}">
...
$get_nm=$this->get_template_vars('module');
$get_nm = $get_nm['module'];
$module_nm=array_search($get_nm,$app_list_strings['moduleList']);  
...
<i class="fa {php}echo (!empty($icon_module[$module_nm]['faicon']))? $icon_module[$module_nm]['faicon']:$icon_module['default']['faicon']; {/php} fa-stack-1x" style="background-color:{php}echo (!empty($icon_module[$module_nm]['bgcolor']))? $icon_module[$module_nm]['bgcolor']:$icon_module['default']['bgcolor']; {/php};color:#FFFFFF;border-radius:5px"></i></span>&nbsp;

<span id="title" style="font-size: 14px" title="{$module.title}">{$module.title|truncate:30:'...':true}</span>
			<!-- <span id="mbLBLL" class="mbLBLL"></span></a></td> -->
		{if $rowCounter % 2 == 1}
		</tr>
		{/if}
		{counter}
