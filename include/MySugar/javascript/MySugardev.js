...
initMySugar = function () {
    SUGAR.mySugar = function () {
        ...
        return{getLayout: function (asString) {
                ...
                configureDlg = new YAHOO.widget.SimpleDialog("dlg", {visible: false, width: "510", effect: [{effect: YAHOO.widget.ContainerEffect.FADE, duration: 0.5}], fixedcenter: true, modal: true, draggable: false});
                ...
                var fillInDashlet = function (data) {
                    ...
                    if (is_chart_dashlet) {
                        var chartScriptObj = YAHOO.util.Connect.asyncRequest('GET', scriptUrl, {success: processChartScript, failure: processChartScript}, null);
                    }
                    $(window).resize();
                }
                ...
                var cObj = YAHOO.util.Connect.asyncRequest('GET', 'index.php?to_pdf=1&module=' + module + '&action=DynamicAction&DynamicAction=addDashlet&activeTab=' + activeTab + '&id=' + id + '&type=' + type + '&type_module=' + encodeURIComponent(type_module), {success: success, failure: success}, null);
                SUGAR.mySugar.sugarCharts.loadSugarCharts();
                return false;
