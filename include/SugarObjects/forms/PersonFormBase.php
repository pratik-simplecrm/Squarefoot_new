...
function checkForDuplicates($prefix='')
{
	...
	if(!empty($emailStr))
    {
        ...
        {
			if(!isset($rows[$row['id']])) {
			   $query2 = "SELECT tn.id, tn.first_name, tn.last_name, tn.title, concat(u.first_name,' ', u.last_name)as Assigned_user_Name FROM {$focus->table_name} AS tn join users u on tn.assigned_user_id = u.id and u.deleted = 0 WHERE tn.deleted=0 AND tn.id = '" . $row['id'] . "'";
