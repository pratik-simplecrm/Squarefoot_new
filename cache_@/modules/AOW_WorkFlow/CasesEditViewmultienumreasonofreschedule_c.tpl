...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
	...
	<input type="hidden" id="{$fields.reasonofreschedule_c.name}_multiselect"
	name="{$fields.reasonofreschedule_c.name}_multiselect" value="true">
	{multienum_to_array string=$fields.reasonofreschedule_c.value default=$fields.reasonofreschedule_c.default assign="values"}
	<select id="{$fields.reasonofreschedule_c.name}"
	name="{$fields.reasonofreschedule_c.name}[]"
	multiple="true" size='6' style="width:150" title='' tabindex="1"  
     	>
	{html_options options=$fields.reasonofreschedule_c.options selected=$values}
	</select>
...
{else}
	...
	{assign var="ac_key" value=$smarty.capture.ac_key}
		...
		name="{$fields.reasonofreschedule_c.name}_multiselect" value="true">
		{multienum_to_array string=$fields.reasonofreschedule_c.value default=$fields.reasonofreschedule_c.default assign="values"}
		<select style='display:none' id="{$fields.reasonofreschedule_c.name}"
		name="{$fields.reasonofreschedule_c.name}[]"
		multiple="true" size='6' style="width:150" title='' tabindex="1"  
		        >
		{html_options options=$fields.reasonofreschedule_c.options selected=$values}
	...
	<span class="id-ff multiple">
	    <button type="button">
	    	<img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.reasonofreschedule_c.name}-image">
	    	</button>
	    	<button type="button"
	        id="btn-clear-{$fields.reasonofreschedule_c.name}-input"
	        title="Clear"
	        onclick="SUGAR.clearRelateField(this.form, '{$fields.reasonofreschedule_c.name}-input', '{$fields.reasonofreschedule_c.name};');SUGAR.AutoComplete.{$ac_key}.inputNode.updateHidden()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
	</span>
	...
	YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters","node-event-simulate", function (Y) {
		...
		if({/literal}SUGAR.AutoComplete.{$ac_key}.minQLen{literal} == 0){
		    ...
		    function updateTextOnInterval(){
		    	if (document.activeElement != document.getElementById("{/literal}{$fields.reasonofreschedule_c.name}-input{literal}"))
