...
initMySugar = function () {
    SUGAR.mySugar = function () {
        ...
        var homepage_dd;
        return{getLayout: function (asString) {
                columns = [];
                for (je = 0; je < 3; je++) {
                ...
                YAHOO.util.Connect.asyncRequest('POST', 'index.php?' + SUGAR.util.paramsToUrl({'module': module, 'action': 'DynamicAction', 'DynamicAction': 'saveLayout', 'selectedPage': activeTab, 'to_pdf': 1}), {success: success, failure: success}, SUGAR.util.paramsToUrl({'layout': order}));
            ...
            configureDashlet: function (id) {
                ...
                /*configureDlg = new YAHOO.widget.SimpleDialog("dlg", {visible: false, effect: [{effect: YAHOO.widget.ContainerEffect.FADE, duration: 0.5}], fixedcenter: false, modal: true, draggable: false, x: 0, zIndex: 11});*/
                ...
                fillInConfigureDiv = function (data) {
                    ...
                    /* Customizations for bootstrap modal by Roshan Sarode */
                    $(".edit-dashlet-modal .modal-title").html(result['header']);
                    //           configureDlg.show();

                    $(".edit-dashlet-modal").modal('toggle');



                    $(".edit-dashlet-modal").closest(".bd").css({"padding": "0px", "overflow-y": "hidden"});
                    /* Customizations for bootstrap modal by Roshan Sarode */
                    ...
                    $('#dlg').animate({top: (newTop) + 'px'});
                };
                SUGAR.mySugar.configureDashletId = id;
                var cObj = YAHOO.util.Connect.asyncRequest('GET', 'index.php?to_pdf=1&module=' + module + '&action=DynamicAction&DynamicAction=configureDashlet&id=' + id, {success: fillInConfigureDiv, failure: fillInConfigureDiv}, null);
...
//                        alert("hello")
               // $(".flip_open").attr("href", "javascript:void(0)");
                ...
                $.ajax({"method": "POST", "url": "index.php?module=" + module + "&action=DynamicAction&DynamicAction=addDashlet&activeTab=" + activeTab + "&id=" + id + "&to_pdf=1", "data": {'type': type, 'type_module': type_module}}).done(function (dashlet_id) {
                   var dashlet_id=trim(dashlet_id);
                 //  alert("--"+dashlet_id+"--");
                ...
                SUGAR.mySugar.dashletsDialog = new YAHOO.widget.Dialog("dashletsDialog", {width: "480px", height: height + "px", fixedcenter: true, draggable: false, visible: false, modal: true, close: false});
                var listeners = new YAHOO.util.KeyListener(document, {keys: 27}, {fn: function () {
