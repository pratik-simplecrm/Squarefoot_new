<?php
   ...
   class opportunities_scoring_after_retrieve_class
   {
      function opportunities_scoring_after_retrieve_method($bean, $event, $arguments)
      {
...
$bean=$bean->retrieve($bean->id);
            ...
            foreach($all_fields as $field=>$marking)
            {
				...
				$bean->opportunities_scoring_c = $tot_mark; 
			}
        if($tot_mark<1)            
        {
                $bean->opportunities_scoring_c =0;	
        }
         //logic
