...
class ViewConvertLead extends SugarView {
    ...
    public function display() {
        //Customizations to auto populate probability by Roshan Sarode 26-4-18 start
        global $app_list_strings;
        $json = getJSONobj();
        $prob_array = $json->encode($app_list_strings['sales_probability_dom']);
        $prePopProb = '';
        $prePopProb = 'document.getElementsByName(\'Opportunitiessales_stage\')[0].onchange();';
        echo $probability_script = <<<EOQ
        <script>
        prob_array = $prob_array;
        $(document).ready(function(){
        $("#Opportunitiessales_stage").change(function(){
        genrate_probability();
        })
        genrate_probability();
        })

        function genrate_probability(){
         var sales_stage= $("#Opportunitiessales_stage").val();
        $('#Opportunitiesprobability').val(prob_array[sales_stage]);    
        }
        </script>
EOQ;
         //Customizations to auto populate probability by Roshan Sarode 26-4-18 end
        echo $js = <<<EOC
<script>
$("#Opportunitiesarch_architectural_firm_opportunities_1_name").prop("readonly",true);
$('#Opportunitiesarch_architectural_firm_opportunities_1_name').css('background-color' , '#DEDEDE')
$('#btn_Opportunitiesarch_architectural_firm_opportunities_1_name').hide();
$('#btn_clr_Opportunitiesarch_architectural_firm_opportunities_1_name').hide();

$("#Opportunitiesarch_architects_contacts_opportunities_1_name").prop("readonly",true);
$('#Opportunitiesarch_architects_contacts_opportunities_1_name').css('background-color' , '#DEDEDE')
$('#btn_Opportunitiesarch_architects_contacts_opportunities_1_name').hide();
$('#btn_clr_Opportunitiesarch_architects_contacts_opportunities_1_name').hide();
</script>
EOC;
            ...
            foreach ($focus->field_defs as $field => $def) {
                if (isset($vdef[$ev->view]['copyData']) && $vdef[$ev->view]['copyData']) {
                    if ($module == "Accounts" && $field == 'name') {
                        ...
                        $focus->arch_architectural_firm_opportunities_1_name = $this->focus->arch_architectural_firm_leads_1_name;
                        $focus->arch_archi003bal_firm_ida = $this->focus->arch_architectural_firm_leads_1arch_architectural_firm_ida;
                        $focus->arch_architects_contacts_opportunities_1_name = $this->focus->arch_architects_contacts_leads_1_name;
                        $focus->arch_archi342contacts_ida = $this->focus->arch_architects_contacts_leads_1arch_architects_contacts_ida;
                        $focus->flooring_type_c = $this->focus->flooring_tpye_c;
                    } else if ($field == "id") {
                        //If it is not a contact, don't copy the ID from the lead
                        if ($module == "Contacts") {

                            $focus->$field = $this->focus->$field;
                            $focus->title = $this->focus->designation_c;
                            // echo "<pre>";
                            //  print_r($this->focus);
                            //  exit;
                        }
    ...
    protected function getRecord() {
        ...
        $this->focus->title = $this->focus->designation_c;
        $this->focus->save();
    }
