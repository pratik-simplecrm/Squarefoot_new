<?php
   ...
   class lead_scoring_process_record_class
   {
      function lead_scoring_process_record_method($bean, $event, $arguments)
      {
...
$content_row = $db->fetchByAssoc($content_query);
	  ...
	  $bean=$bean->retrieve($bean->id);
            ...
            foreach($all_fields as $field=>$marking)
            {
				...
				$bean->lead_scoring_c = "<span class='label ".$label."' style='display: inline-block;min-width: 30px;padding: 8px 8px;font-size: 12px;font-weight: 700;line-height: 1;color: #fff;text-align: center;white-space: nowrap;vertical-align: middle;border-radius: 50px;'>".$tot_mark."</span>"; 
			}
        if($tot_mark<1)            
        {
                $bean->lead_scoring_c="<span class='label label-danger' style='display: inline-block;min-width: 30px;padding: 8px 8px;font-size: 12px;font-weight: 700;line-height: 1;color: #fff;text-align: center;white-space: nowrap;vertical-align: middle;border-radius: 50px;'>0</span>";	
        }
         //logic
      }
   }
