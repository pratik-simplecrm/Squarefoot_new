...
class bhea_ReportsViewgeneratedailyarchitectreport extends SugarView{
    ...
    function display()
	{
        ...
        if($_REQUEST['Export_Chart'])
        {
           ...
           /* CAT:Bar Chart */
            ...
            $pdf->SetAuthor('BHEA');
             $report_name =  "Userwise Daily Architect Report By ".$_REQUEST['group_by'][0];
...
D;
		...
		$report_name = "Userwise Daily Architect";
			...
			$(document).ready(function(){
				...
				$('#Schedule_Report').click(function(){				
					$.ajax({
					  ...
					  type:"POST",
						data:{module:'Bhea_Report_Scheduler',action:'EditView',return_module:'bhea_Reports',return_action:'generateDailyArchitectReport',to_pdf:'true'}
					  }).done(function(data){
...
HTML_Search;
		...
		<div id="mainData">
			...
			<tr  height="25">
			<th><label>Name&nbsp;&nbsp;<img border="0" align="absmiddle" src="themes/default/images/arrow_down.gif"></lable></th>
			<th><label>Type</lable></th>
			<th><label>Mobile Phone</lable></th>
			<th ><label>Email Address</lable></th>
...
HTML_Data_header;
        ...
        function setPageBack(index)
        {
            ...
            {
                $('#imageLoading').css('display','block');
                url =  "index.php?module=bhea_Reports&action=AjaxDataArchitect&to_pdf=1&curr_index="+index; 
            ...
            else
            {
                ...
                if(curr_index > 50) {
                    ...
                    url =  "index.php?module=bhea_Reports&action=AjaxDataArchitect&to_pdf=1&curr_index="+(parseInt(prev_index)+1);
                    $.ajax({
                        type: "POST",
                        url: url,
                        data: datastring,
		...
		function setPage(index)
		{
            ...
            if(index == 'end') 
            {
                ...
                url =  "index.php?module=bhea_Reports&action=AjaxDataArchitect&to_pdf=1&curr_index="+curr_index; 
...
JS;
		...
		if(!empty($_REQUEST['Export']))
		{
			...
			header('Content-Disposition: attachment; filename=Architect.csv');

			// create a file pointer connected to the output stream
			$output = fopen('php://output', 'w');

			// output the column headings
			fputcsv($output, array('Name', 'Type','Mobile Phone','Email Address','Architectural Firm Name','Assigned to','Team'));
