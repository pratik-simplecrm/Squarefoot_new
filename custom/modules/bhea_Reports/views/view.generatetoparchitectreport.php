...
class bhea_ReportsViewgeneratetoparchitectreport extends SugarView{
    ...
    function getChartData($post)
    {
       ...
       if ($group_by == 'Assigned User'){
         ...
         $queryChart = " SELECT
            $group_by_query , count(AR.id) as total
	...
	function getMatrixData($post)
	{
	    ...
	    $query = "SELECT AR.id,
					...
					COUNT(AR.id) archi_architects_contact__allcount,
					COUNT(DISTINCT  AR.id) archi_architects_contact__count,
					sum(IFNULL(O.amount,0)/IFNULL(cur.conversion_rate,1))*1 sum_amount

					FROM arch_architects_contacts AR
					LEFT OUTER JOIN arch_architects_contacts_opportunities_1_c ARO ON AR.id=ARO.arch_archi342contacts_ida AND ARO.deleted=0

					LEFT OUTER JOIN opportunities O ON O.id=ARO.arch_architects_contacts_opportunities_1opportunities_idb AND O.deleted=0
					LEFT JOIN currencies cur ON O.currency_id=cur.id AND cur.deleted=0
					 WHERE ((((coalesce(LENGTH(O.name),0) > 0)
					))) 
					AND  AR.deleted=0 
					AND  CONCAT(IFNULL(AR.first_name,''),' ',IFNULL(AR.last_name,'')) NOT LIKE 'No Architect'
					GROUP BY archi_name 
					ORDER BY sum_amount DESC limit 0 , 10 ";
		...
		{
			$archi_id 		= $row['id'];
			$count 			= $row['archi_architects_contact__allcount'];
			$sum_amount		= $row['sum_amount'];
			$archi_name 	= ucwords(strtolower($row['archi_name']));
			
			$query2 = "SELECT IFNULL(O.id,'') opp_id,
					...
					AND AR.id ='$archi_id'
						...
						ORDER BY archi_name  ASC ";
					$result2 = $db->query($query2);
					
					$r2 = 1;
					while($row2 = $db->fetchByAssoc($result2))
					{
						$data[$r][$r2]['opp_id'] 	= $row2['opp_id'];
						$data[$r][$r2]['opp_name'] 	= ucwords(strtolower($row2['opp_name']));
						$data[$r][$r2]['opp_amount'] = $row2['opp_amount'];		
						
						$data[$r][$r2]['arc_id'] = $row['id'];	
						$data[$r][$r2]['count']  = $row['archi_architects_contact__allcount'];
						$data[$r][$r2]['archi_name'] = ucwords(strtolower($row['archi_name']));
						$data[$r][$r2]['sum_amount'] = $row['sum_amount'];
						$r2++;
					}
			$r++;
		}
    ...
    function display()
	{
        ...
        if($_REQUEST['Export_Chart'])
        {
           ...
           /* CAT:Bar Chart */
            ...
            $pdf->SetAuthor('BHEA');
             $report_name =  "Userwise Daily Quote Report By ".$_REQUEST['group_by'][0];
		...
		if(!empty($MData))
		{
			foreach($MData as $d) 
			{ 
			//echo "<pre>";print_r($d[1]['archi_name']);exit;
			$data .= '<table cellpadding="0" cellspacing="0" width="100%" id="table_data">
			<tr height="25">
			<th bgcolor ="#bab4b4">Architect Full Name ='.$d[1]['archi_name'].' ,  Count = '.$d[1]['count'].' ,  SUM: Opportunity Amount = '.$d[1]['sum_amount'].'</th><th bgcolor ="#bab4b4"></th>
			</tr>
			<tr  height="25">
			  ...
			  foreach($d as $p) {
				...
				$data .= '<tr height="25">';
				$data .= '<td><label><b><a href="index.php?module=Opportunities&action=DetailView&record='.$p['opp_id'].'">'.$p['opp_name'].'</a></b></lable></td>';
				$data .= '<td><label>'.$p['opp_amount'].'</lable></td>';
...
D;
		...
		$report_name = "Userwise Daily Quote";
...
HTML_Data_header;
        ...
        function setPageBack(index)
        {
            ...
            {
                $('#imageLoading').css('display','block');
                url =  "index.php?module=bhea_Reports&action=AjaxDataTopArchitect&to_pdf=1&curr_index="+index; 
            ...
            else
            {
                ...
                if(curr_index > 50) {
                    ...
                    url =  "index.php?module=bhea_Reports&action=AjaxDataTopArchitect&to_pdf=1&curr_index="+(parseInt(prev_index)+1);
                    $.ajax({
                        type: "POST",
                        url: url,
                        data: datastring,
		...
		function setPage(index)
		{
            ...
            if(index == 'end') 
            {
                ...
                url =  "index.php?module=bhea_Reports&action=AjaxDataTopArchitect&to_pdf=1&curr_index="+curr_index; 
...
JS;
		...
		if(!empty($_REQUEST['Export']))
		{
			...
			fputcsv($output, array('Customer Name',	'Opportunity Amount', 'Billing City', 'Billing Country', 'Type', 'Assigned User'));
