...
class bhea_ReportsViewoppbybranch extends SugarView{
    ...
    function display(){
		...
		$query = "SELECT p.id, p.name
				  FROM quote_products p join quote_products_cstm pc on p.id=pc.id_c 
				  WHERE p.deleted=0";
...
BOC;
    ...
    function oppByBranch(){

		global $db;
		$stReturnString = '';

		$groupBy = 'GROUP BY bd.name';
		$count = ', count(*) total_count';
		$query_res = $this->sqlResult($groupBy,$count);
		if($query_res->num_rows > 0) {
			while($row = $db->fetchByAssoc($query_res)) {
				$arLeadChartDat[$row['branch_name']] = $row['total_count'];
			}
		}
		...
		$obSugarChart->setProperties('Opportunity By Branch', '', $this->chartDefs['chartType']);
		$obSugarChart->base_url=$this->chartDefs['base_url'];
		$obSugarChart->url_params = array();

		$xmlFile = $obSugarChart->getXMLFileName('opp_by_branch');
		$obSugarChart->saveXMLFile($xmlFile, $obSugarChart->generateXML());
		$stReturnString=$obSugarChart->display('opp_by_branch', $xmlFile, "100%", '480');
...
function runReport()
{
	...
	if(gt == 'Run Report')
	{
		document.frmsales.action='index.php?module=bhea_Reports&action=opportunityByBranch';
