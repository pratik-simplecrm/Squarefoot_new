...
class bhea_ReportsViewgenerateloanreport extends SugarView {
    ...
    function display(){
		...
		$query = "SELECT name, unit from bhea_demographics AS B JOIN bhea_demographics_cstm AS BC ON B.id=BC.id_c WHERE B.deleted=0 ";
		$query_res = $db->query($query);
		...
		$child_loan = $_REQUEST['child_loan'];
		if($child_loan == 'child_loan')
		{
			$check_value='checked';
		}else {
			$check_value='';
		...
		echo $html = <<<BOC
		<form name="frmloan" id="frmloan" action="index.php?module=bhea_Reports&action=generateLoanReport" method="post">
		<p style="font-size:15px;padding:10px 0px;10px;0px;font-weight:bold">Loan Report</p>
		<body >
		<table width="100%" border="0" align="center" id="oppReportTable">
			...
			<tr>	
				...
				<td>
					<b>Child Loan:</b> <input type="checkbox" id="child_loan" name="child_loan" value="child_loan" $check_value>
...
BOC;
		...
		if($query_res->num_rows > 0) {
			...
			while($row = $db->fetchByAssoc($query_res)) {		
				...
				$loan_amt       = number_format($row['loan_amount'],2,'.',',');
	...
	function sqlResult($groupBy=NULL, $count=NULL) {	
		...
		$check = $_REQUEST['child_loan'];
		
		if($check == 'child_loan') {
			$child_loan_condition = "AND l.id in (	SELECT LL.id
									FROM bhea_loans AS LL 

									JOIN bhea_loans_bhea_loans_c AS LLR ON LL.id=LLR.bhea_loans_bhea_loansbhea_loans_idb
...
function runReport()
{
	...
	if(gt == 'Run Report')
	{
		document.frmloan.action='index.php?module=bhea_Reports&action=generateLoanReport';
