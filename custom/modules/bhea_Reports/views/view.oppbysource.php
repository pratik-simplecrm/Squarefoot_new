...
class bhea_ReportsViewoppbysource extends SugarView{
    ...
    function display(){
		...
		<a href="$site_url/index.php?module=bhea_Reports&action=opportunityBySource">Opportunity by Source </a>&nbsp;&nbsp;
		<a href="$site_url/index.php?module=bhea_Reports&action=opportunityByProduct">Opportunity by Product </a>&nbsp;&nbsp;
		<a href="$site_url/index.php?module=bhea_Reports&action=opportunityByBranch">Opportunity by Branch </a>
		</p>
		<body >
...
BOC;
    ...
    function oppBySource(){
		global $db;

		$stReturnString = '';
		$groupBy = "GROUP BY o.lead_source ORDER BY o.lead_source";
		$count = ", count(*) total_count";

		$query_res = $this->sqlResult($groupBy, $count);
		if($query_res->num_rows > 0) {
			while($row = $db->fetchByAssoc($query_res)) {
				if($row['lead_source'] == '') $row['lead_source']='-None-';
				$arLeadChartDat[$row['lead_source']] = $row['total_count'];
			}
		}

		$obSugarChart = SugarChartFactory::getInstance();
		$sugarChart->group_by = array('ss','lead_source');

		$obSugarChart->setData($arLeadChartDat);
		$obSugarChart->setProperties('Opportunity By Source', '', $this->chartDefs['chartType']);
		$obSugarChart->base_url=$this->chartDefs['base_url'];
		$obSugarChart->url_params = array();

		$xmlFile = $obSugarChart->getXMLFileName('opp_by_leadsource');
		$obSugarChart->saveXMLFile($xmlFile, $obSugarChart->generateXML());
		$stReturnString=$obSugarChart->display('opp_by_leadsource', $xmlFile, "100%", '480');
...
function runReport()
{
	...
	if(gt == 'Run Report')
	{
		document.frmsales.action='index.php?module=bhea_Reports&action=opportunityBySource';
...
function exportExcel() {
	...
	$('body').prepend("<form method='post' action='custom_export.php' style='display:none' id='ReportTableData'><input type='hidden' name='report_name' value='Opportuntiy' /><input type='text' name='tableData' value='"+data+"' ></form>");
