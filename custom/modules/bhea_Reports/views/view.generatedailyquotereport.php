...
class bhea_ReportsViewgeneratedailyquotereport extends SugarView{
    ...
    function getChartData($post)
    {
       ...
       if ($group_by == 'Assigned User'){
           ...
           ;
         $queryChart = " SELECT
            $group_by_query , count(Q.id) as total
			...
			$(document).ready(function(){
				...
				$('#Schedule_Report').click(function(){				
					$.ajax({
					  ...
					  type:"POST",
						data:{module:'Bhea_Report_Scheduler',action:'EditView',return_module:'bhea_Reports',return_action:'generateDailyQuoteReport',to_pdf:'true'}
					  }).done(function(data){
...
HTML_Search;
		...
		<div id="mainData">
			...
			<tr  height="25">
			<th><label>Quote Subject&nbsp;&nbsp;<img border="0" align="absmiddle" src="themes/default/images/arrow_down.gif"></lable></th>
			<th><label>Quote Stage</lable></th>
			<th><label>Total</lable></th>
			<th ><label>Opportunity Name</lable></th>
			<th ><label>Valid Until</lable></th>
...
HTML_Data_header;
        ...
        function setPageBack(index)
        {
            ...
            {
                $('#imageLoading').css('display','block');
                url =  "index.php?module=bhea_Reports&action=AjaxDataQuote&to_pdf=1&curr_index="+index; 
            ...
            else
            {
                ...
                if(curr_index > 50) {
                    ...
                    url =  "index.php?module=bhea_Reports&action=AjaxDataQuote&to_pdf=1&curr_index="+(parseInt(prev_index)+1);
                    $.ajax({
                        type: "POST",
                        url: url,
                        data: datastring,
		...
		function setPage(index)
		{
            ...
            if(index == 'end') 
            {
                ...
                url =  "index.php?module=bhea_Reports&action=AjaxDataQuote&to_pdf=1&curr_index="+curr_index; 
...
JS;
		...
		if(!empty($_REQUEST['Export']))
		{
			...
			fputcsv($output, array('Quote Subject', 'Quote Stage','Total','Opportunity Name','Valid Until','Assigned to','Team'));
