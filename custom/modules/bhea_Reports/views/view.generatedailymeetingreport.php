...
class bhea_ReportsViewgeneratedailymeetingreport extends SugarView{
    ...
    function getChartData($post)
    {
        ...
        if(!empty($assigned_users_team))
        {
            if(is_array($assigned_users_team))
            {
                ...
                if(!empty($tmp))
                    $assigned_users_team = " and M.team_id in ($tmp) ";
                else
                    $assigned_users_team = '';
            } 
        }
        ...
        if(!empty($assigned_users))
        {
            if(is_array($assigned_users))
            {
                ...
                if(!empty($tmp))
                    $assigned_users = " and M.assigned_user_id in ($tmp) ";
                else
                    $assigned_users = '';
            } 
        }
       ...
       if ($group_by == 'Assigned User'){
           ...
           ;
         $queryChart = " SELECT
            $group_by_query , count(M.id) as total
			
		FROM meetings M
			LEFT JOIN  users U ON M.assigned_user_id=U.id AND U.deleted=0 AND U.status ='Active' 
			...
			LEFT JOIN team as T ON T.id=M.team_id and T.deleted=0
	...
	function getMatrixData($post)
	{
	    ...
	    $query = " SELECT IFNULL(M.id,'') id,
				...
				WHERE M.deleted=0 
					...
					ORDER BY M.date_entered DESC    $limit
				";
		...
		{
			$data[$r]['id'] 				= $row['id'];
			$data[$r]['subject'] 	   		= ucwords(strtolower($row['meetings_name']));
			...
			$data[$r]['meetings_description'] 	= $row['meetings_description'];
    ...
    function display()
	{
        ...
        if($_REQUEST['Export_Chart'])
        {
           ...
           /* CAT:Bar Chart */
            ...
            $pdf->SetAuthor('BHEA');
             $report_name =  "Userwise Daily Meeting Report By ".$_REQUEST['group_by'][0];
		...
		if(!empty($MData))
		{
			...
			{
				$data .= '<tr height="25">';
				$data .= '<td><label><b><a href="index.php?module=Meetings&action=DetailView&record='.$d['id'].'">'.$d['subject'].'</a></b></lable></td>';
				...
				$data .= '<td><label>'.$d['meetings_description'].'</lable></td>';
...
D;
		...
		$report_name = "Userwise Daily Meeting";
			...
			$(document).ready(function(){
				...
				$('#Schedule_Report').click(function(){				
					$.ajax({
					  ...
					  type:"POST",
						data:{module:'Bhea_Report_Scheduler',action:'EditView',return_module:'bhea_Reports',return_action:'generateDailyMeetingReport',to_pdf:'true'}
					  }).done(function(data){
