  ...
  function callme($assigned_user_id)
  {
	...
	while($rec_user = $db->fetchByAssoc($row_user)){
		...
		if(!empty($team_user_list)){
			$user_team_list = " AND SG.securitygroup_id IN ('" . implode("','",$team_user_list) . "') AND SG.module = 'quote_Quote' ";
		}
		else{
			$user_team_list ='';	
		}
	...
	$query = " SELECT 
				count(Q.id) as total_rows,SG.securitygroup_id
				FROM quote_quote Q
        ...
        if($row = $db->fetchByAssoc($result))
        {
            ...
            $security_team_id = $row['securitygroup_id'];
            
            $eachRocordTeamName ="SELECT name FROM securitygroups where id='$security_team_id' ";
				$result1=$db->query($eachRocordTeamName);
