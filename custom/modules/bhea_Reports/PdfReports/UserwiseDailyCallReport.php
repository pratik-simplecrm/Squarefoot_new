  ...
  function callme($assigned_user_id)
  {
	...
	while($rec_user = $db->fetchByAssoc($row_user)){
	 ...
	 $get_user_report="SELECT RS.assigned_user_id AS assigned_user_id,U.user_name ,U.is_admin
		...
		if(!empty($team_user_list)){
			$user_team_list = " AND SG.securitygroup_id IN ('" . implode("','",$team_user_list) . "') AND SG.module = 'Calls' ";
		}
		else{
			$user_team_list ='';	
		}
	...
	$query = " SELECT 
				...
				FROM 
					...
					LEFT JOIN securitygroups_records SG ON C.id = SG.record_id and SG.deleted=0
				WHERE C.deleted=0  
					$user_team_list
					$from_date
					$to_date	 
