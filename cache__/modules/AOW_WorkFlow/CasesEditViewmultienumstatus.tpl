...
{else}
	...
	{assign var="ac_key" value=$smarty.capture.ac_key}
		...
		name="{$fields.status.name}_multiselect" value="true">
		{multienum_to_array string=$fields.status.value default=$fields.status.default assign="values"}
		<select style='display:none' id="{$fields.status.name}"
		name="{$fields.status.name}[]"
		multiple="true" size='6' style="width:150" title='' tabindex="1"  
		        >
		{html_options options=$fields.status.options selected=$values}
	...
	<span class="id-ff multiple">
	    <button type="button">
	    	<img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.status.name}-image">
	    	</button>
	    	<button type="button"
	        id="btn-clear-{$fields.status.name}-input"
	        title="Clear"
	        onclick="SUGAR.clearRelateField(this.form, '{$fields.status.name}-input', '{$fields.status.name};');SUGAR.AutoComplete.{$ac_key}.inputNode.updateHidden()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
	</span>
	...
	YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters","node-event-simulate", function (Y) {
		...
		if({/literal}SUGAR.AutoComplete.{$ac_key}.minQLen{literal} == 0){
		    ...
		    function updateTextOnInterval(){
		    	if (document.activeElement != document.getElementById("{/literal}{$fields.status.name}-input{literal}"))
