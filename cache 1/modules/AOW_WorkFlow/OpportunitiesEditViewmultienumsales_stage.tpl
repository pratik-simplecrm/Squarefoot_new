...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
	...
	<input type="hidden" id="{$fields.sales_stage.name}_multiselect"
	name="{$fields.sales_stage.name}_multiselect" value="true">
	{multienum_to_array string=$fields.sales_stage.value default=$fields.sales_stage.default assign="values"}
	<select id="{$fields.sales_stage.name}"
	name="{$fields.sales_stage.name}[]"
	multiple="true" size='6' style="width:150" title='' tabindex="1"  
     	>
	{html_options options=$fields.sales_stage.options selected=$values}
	</select>
...
{else}
	...
	{assign var="ac_key" value=$smarty.capture.ac_key}
		...
		name="{$fields.sales_stage.name}_multiselect" value="true">
		{multienum_to_array string=$fields.sales_stage.value default=$fields.sales_stage.default assign="values"}
		<select style='display:none' id="{$fields.sales_stage.name}"
		name="{$fields.sales_stage.name}[]"
		multiple="true" size='6' style="width:150" title='' tabindex="1"  
		        >
		{html_options options=$fields.sales_stage.options selected=$values}
	...
	<span class="id-ff multiple">
	    <button type="button">
	    	<img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.sales_stage.name}-image">
	    	</button>
	    	<button type="button"
	        id="btn-clear-{$fields.sales_stage.name}-input"
	        title="Clear"
	        onclick="SUGAR.clearRelateField(this.form, '{$fields.sales_stage.name}-input', '{$fields.sales_stage.name};');SUGAR.AutoComplete.{$ac_key}.inputNode.updateHidden()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
	</span>
	...
	YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters","node-event-simulate", function (Y) {
		...
		if({/literal}SUGAR.AutoComplete.{$ac_key}.minQLen{literal} == 0){
		    ...
		    function updateTextOnInterval(){
		    	if (document.activeElement != document.getElementById("{/literal}{$fields.sales_stage.name}-input{literal}"))
